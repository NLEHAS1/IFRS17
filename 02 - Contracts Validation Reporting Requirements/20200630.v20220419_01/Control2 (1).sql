...
Create view MI2022.Control2_20200630_v20220419_01 as
Select
distinct
  a.*,
...
from
  ...
  count(contractid)
from
  MI2022.Contracts_SL1_20200630_v20220419_01
where 
  DataSource='SYM'
  and RiskPeriodEndDate>20200630
group by
  policyid,
  ContractIssueDate
having
  count(contractid) > 12) b
  left join MI2022.Contracts_SL1_20200630_v20220419_01 a on (b.PolicyId=a.PolicyId and b.ContractIssueDate=a.ContractIssueDate)
  left join (
    SELECT
      a.ID POLICY_ID,
      a.CMPCT_ID CMPCT_ID,
    ...
    FROM
      MI2022.TBBU_POLICIES_SL1_20200630_v20220419_01 a
      left join MI2022.TBPO_POL_MODULES_SL1_20200630_v20220419_01 b on a.ID = b.BUPIY_ID --AND d_popss_status_code in ('LIVE', 'SUSP')
      AND (
  ...
  left join MI2022.TBPO_POL_VERSIONS_SL1_20200630_v20220419_01 d on (a.policyid = d.bupiy_id and to_date(d.start_risk_dat, 'yyyyMMdd') = to_date(cast(a.ContractIssueDate as string), 'yyyyMMdd')) 
  left join  MI2022.TBPO_POL_MOD_VARIABLES_SL1_20200630_v20220419_01 c on (a.policyid=c.bupiy_id  and Current_Date() between c.effect_from_dat and c.effect_to_dat and c.porve_code IN ('RAFLG', 'LOFLG')) 
  ...
  Left Join (SELECT
            ...
            FROM MI2022.TBPO_POL_VERSIONS_SL1_20200630_v20220419_01 pol
...
select count(*) from MI2022.Control2_20200630_v20220419_01 
