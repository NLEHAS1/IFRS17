...
Create view MI2022.Control2_20191231_v20220529_01 as
Select
distinct
  a.*,
...
where 
  DataSource='SYM'
  and CoverEndDate >= 20200101
group by
  policyid,
  ContractIssueDate
having
  count(contractid) > 12) b
  left join MI2022.Contracts_SL1_20191231_v20220529_01 a on (b.PolicyId=a.PolicyId and b.ContractIssueDate=a.ContractIssueDate)
  left join (
    SELECT
      a.ID POLICY_ID,
      a.CMPCT_ID CMPCT_ID,
    ...
    FROM
      ...
      left join MI2022.TBPO_POL_MODULES_SL1_20191231_v20220529_01 b on a.ID = b.BUPIY_ID --AND d_popss_status_code in ('LIVE', 'SUSP')
      AND (
  ...
  left join MI2022.TBPO_POL_VERSIONS_SL1_20191231_v20220529_01 d on (a.policyid = d.bupiy_id and to_date(d.start_risk_dat, 'yyyyMMdd') = to_date(cast(a.ContractIssueDate as string), 'yyyyMMdd')) 
  left join  MI2022.TBPO_POL_MOD_VARIABLES_SL1_20191231_v20220529_01 c on (a.policyid=c.bupiy_id  and Current_Date() between c.effect_from_dat and c.effect_to_dat and c.porve_code IN ('RAFLG', 'LOFLG')) 
  ...
  Left Join (SELECT
            ...
            FROM MI2022.TBPO_POL_VERSIONS_SL1_20191231_v20220529_01 pol
            JOIN MI2022.TBPO_CTRY_GRP_MCTS_SL1_20191231_v20220529_01 ctrygrp on (ctrygrp.bupiy_id=pol.Bupiy_id AND pol.end_risk_dat BETWEEN ctrygrp.effect_from_dat AND ctrygrp.effect_to_dat)
  ...
  left join (select * from (Select
            ...
            From MI2022.TBBU_POLICIES_SL1_20191231_v20220529_01 a
            left join MI2022.TBPO_POL_MODULES_SL1_20191231_v20220529_01 pmod on pmod.bupiy_id=a.id
            Left  Join MI2022.TBPO_REF_MODULE_VARIABLE_TYPES_SL1_20191231_v20220529_01 mref
            On pmod.porme_code = mref.porme_code
            And pmod.porce_code = mref.Porce_code
            Left  Join MI2022.TBPO_POL_MOD_VARIABLES_SL1_20191231_v20220529_01 mvar
