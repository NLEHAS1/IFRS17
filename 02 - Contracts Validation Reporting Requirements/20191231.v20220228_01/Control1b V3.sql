...
select * from Mi2022.Control1b where policyid=215547 
...
select * from MI2022.TBPO_POL_MOD_VARIABLES_SL1_20191231_v20220228_01 pvar
  Left Join MI2022.TBPO_REF_MODULE_VARIABLE_TYPES_SL1_20191231_v20220228_01 refmodvar On pvar.porve_code = refmodvar.porve_code where bupiy_id=215547
...
Select
  ...
  From
    ...
    Left Join MI2022.TBPO_REF_MODULE_VARIABLE_TYPES_SL1_20191231_v20220228_01 refmodvar On pvar.porve_code = refmodvar.porve_code
  Where
    pvar.porve_Code In ('NOMONT03', '5001MONT') and (refmodvar.porme_code LIKE ('05000%')
    OR refmodvar.porme_code LIKE ('05001%')
    OR refmodvar.porme_code LIKE ('05005%')
    OR refmodvar.porme_code LIKE ('0510%')
    OR refmodvar.porme_code LIKE ('%05100%')
...
select * from MI2022.TBPO_POL_MOD_VARIABLES_SL1_20191231_v20220228_01 where porve_Code In ('NOMONT03', '5001MONT')

-- COMMAND ----------

select distinct porme_code from MI2022.TBPO_REF_MODULE_VARIABLE_TYPES_SL1_20191231_v20220228_01  where
porme_code LIKE ('0500%')
...
from
  ...
  Left Join MI2022.TBPO_REF_MODULE_VARIABLE_TYPES_SL1_20191231_v20220228_01 refmodvar On pvar.porve_code = refmodvar.porve_code
where
  ( refmodvar.porme_code LIKE ('05001%')
    OR refmodvar.porme_code LIKE ('05005%')
    OR refmodvar.porme_code LIKE ('0510%')
    OR refmodvar.porme_code LIKE ('%05100%')
     )
  and refmodvar.porve_Code In ('NOMONT03', '5001MONT')
  and bupiy_id in (
    select
      policyid
    from
  ...
  -- Add a condition in case the policies 
...
with RunoffCover as (Select
  pmod.Bupiy_Id as PolicyId,
  pmod.Porce_code,
  pmod.porme_code,
  mvar.PORVE_CODE,
  mvar.value,
  mvar.last_update_dat,
  start_risk_dat
From
  MI2022.TBPO_POL_MODULES_SL1_20191231_v20220228_01 pmod
  inner Join MI2022.TBPO_REF_MODULE_VARIABLE_TYPES_SL1_20191231_v20220228_01 mref On pmod.porme_code = mref.porme_code
  inner Join MI2022.TBPO_POL_MOD_VARIABLES_SL1_20191231_v20220228_01 mvar On (
    pmod.Bupiy_Id = mvar.Bupiy_Id
    And mref.Porve_code = mvar.porve_Code
  )
  inner Join MI2022.TBPO_POL_VERSIONS_SL1_20191231_v20220228_01 pol on (
    pol.bupiy_Id = mvar.bupiy_Id
    and pol.end_risk_dat BETWEEN mvar.effect_from_dat
    AND mvar.effect_to_dat
  )
Where
  (
  ...
  and mref.porve_Code In ('NOMONT03', '5001MONT') 
Group By
  pmod.bupiy_Id,
  pmod.porce_code,
  pmod.porme_code,
  ...
  start_risk_dat
Having
  mvar.last_update_dat = max(mvar.last_update_dat))
  
  select  distinct porme_code from MI2022.Control1b a left join RunoffCover h on (
    a.policyid = h.PolicyId
    and to_date(h.start_risk_dat, 'yyyyMMdd') = to_date(cast(a.ContractIssueDate as string), 'yyyyMMdd')
  ) 
...
RunoffCover as (
  Select
  pmod.Bupiy_Id as PolicyId,
  pmod.Porce_code as RunoffCoverPorceCode,
  pmod.porme_code as RunoffCoverPormeCode,
  mvar.PORVE_CODE as RunoffCoverPorveCode ,
  mvar.value as RunoffCoverValue,
  mvar.last_update_dat as RunoffCoverLastUpdateDat,
  start_risk_dat
From
  MI2022.TBPO_POL_MODULES_SL1_20191231_v20220228_01 pmod
  inner Join MI2022.TBPO_REF_MODULE_VARIABLE_TYPES_SL1_20191231_v20220228_01 mref On pmod.porme_code = mref.porme_code
...
select distinct RunoffCoverPormeCode from mi2022.control1b
