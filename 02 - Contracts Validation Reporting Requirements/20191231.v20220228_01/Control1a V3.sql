...
-- MAGIC %md Comments:
-- MAGIC 1. The logic of the code is to select the maximum value of max_credit_terms_per because we have several values pol.end_risk_dat BETWEEN ctrygrp.effect_from_dat AND ctrygrp.effect_to_dat.
-- MAGIC 2. Include StartRiskDate in the CTE query to be able to join it with the right Policy version in the Contracts table
-- MAGIC 3.The data includes duplicates (around 100 reocrds)
...
select 
a.*,maximumCreditTerms,MaximumCreditTermsType
from
  ...
  where  RiskPeriodStartDate>20191231 and DataSource='SYM'
...
-- MAGIC %md 
-- MAGIC Comments:
-- MAGIC 1. The logic of the code is to select the maximum value of max_cred_per because we have several values pol.end_risk_dat BETWEEN ctrygrp.effect_from_dat AND ctrygrp.effect_to_dat.
-- MAGIC 2. Include StartRiskDate in the CTE query to be able to join it with the right Policy version in the Contracts table
-- MAGIC 3.The data don't includes duplicates
-- MAGIC 
-- MAGIC 4. The MaximumCreditPeriod and MaximumCreditPeriodTermsType includes many nulls from source systems

-- COMMAND ----------

with MaximumCreditPeriod as (
  SELECT
...
select * from MI2022.Contracts_SL1_20191231_v20220228_01 where policyid=29748 and RiskPeriodEndDate > 20191231
  And DataSource = 'SYM'
